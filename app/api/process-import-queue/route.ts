...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const maxTasksPerRun = 5 // 外部调用可以处理更多任务
    ...
    console.log(`🎉 外部API调用完成，处理了 ${processedTasks} 个任务`)
    ...
    return NextResponse.json({ 
      ...
      remainingTasks: Math.max(0, pendingTasks.length - processedTasks),
      timestamp: new Date().toISOString(),
      nextCallRecommended: pendingTasks.length > maxTasksPerRun
    })
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const blob of blobs) {
      try {
        ...
        if (response.ok) {
          ...
          if (task.status === 'pending' || task.status === 'processing') {
            // 如果指定了特定任务ID，只处理这些任务
            if (taskIds.length === 0 || taskIds.includes(task.taskId)) {
              pendingTasks.push(task)
            }
          }
        }
    ...
    let processedTasks = 0
    const tasksToProcess = pendingTasks.slice(0, Math.min(maxTasks, 10)) // 最多10个
...
async function processImportTask(task: any) {
  ...
  // 每次处理的章节数（外部调用可以更多）
  const batchSize = 15 // 比CronJob稍多一些
  ...
  logs.push(`📖 外部API调用 - 开始处理批次: ${chaptersToProcess.length} 个章节`)
  ...
  const updatedTask = {
    ...
    logs,
    updatedAt: new Date().toISOString(),
    lastExternalCall: new Date().toISOString() // 记录外部调用时间
  }
...
async function fetchChapterContentWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 1) {
        const delay = Math.min(Math.pow(2, attempt) * 1000, 8000) // 外部调用重试更快
...
async function fetchChapterContent(chapterUrl: string): Promise<string> {
  const response = await fetch(chapterUrl, {
    ...
    signal: AbortSignal.timeout(25000) // 外部调用稍微短一些超时
  })
